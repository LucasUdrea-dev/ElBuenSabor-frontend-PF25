...
export default function AdminMostrarManufacturado({
  ...
  const calcularCostoTotal = () => {
    let total = 0;
    (articulo?.detalleInsumos ?? []).forEach((detalle) => {
      ...
      <div className="bg-white rounded-4xl w-2/3 m-auto">
        ...
        <div className="p-[5%] *:mb-10">
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <div className="flex gap-2">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      ></div>
                      <h1>
                        {articulo?.existe ? "Disponible" : "No Disponible"}
                      </h1>
                    </div>
                  </div>
                </div>
          ...
          <div className="*:border-gray-500 *:border *:border-collapse text-gray-500 text-center *:py-2">
            ...
            {(articulo?.detalleInsumos?.length ?? 0) > 0 &&
              (articulo?.detalleInsumos ?? []).map((detalle, index) => {
                const cantidad = detalle?.cantidad ?? 0;
                const unidad =
                  detalle?.articuloInsumo?.unidadMedida?.unidad ?? "";
                const nombre = detalle?.articuloInsumo?.nombre ?? "";
                const costo = detalle?.articuloInsumo?.precioCompra ?? 0;
