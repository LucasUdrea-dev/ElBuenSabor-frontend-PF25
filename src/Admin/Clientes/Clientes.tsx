...
export default function Clientes() {
  ...
  const borradoLogicoCliente = async (cliente: Cliente) => {
    ...
    try {
      // Si usás API:
      // await axios.put(`${host}/api/cliente/${cliente.id}`, clienteActualizado);

      // Simulación local
      setClientes((prevClientes) =>
        prevClientes.map((c) => (c.id === cliente.id ? clienteActualizado : c))
      );
              ...
              clientesMostrados.map((cliente, index) => {
                if (
                  ...
                  return (
                    <div
                      key={cliente.id}
                      className={`text-3xl w-full grid grid-cols-[1fr_1.5fr_1fr_0.7fr_1fr] *:border-1 *:border-r-0 *:border-gray-500 *:w-full *:p-5 *:border-collapse text-center *:flex *:items-center *:justify-center font-lato  ${
                        !cliente.activo ? "opacity-40" : ""
                      }`}
                    >
                      <div>
