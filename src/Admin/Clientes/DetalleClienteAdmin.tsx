...
import { host, Usuario, Telefono, Direccion, Pedido, EstadosPedidosEnum } from "../../../ts/Clases";
import LogoUser from "../../../public/svg/LogoUser.svg";
import LogoDetalle from "../../../public/svg/LogoDetalle.svg";
import LogoFactura from "../../../public/svg/DetallePreparacion.svg";
...
const DetalleClienteAdmin: React.FC<Props> = ({ cliente, isOpen, onClose }) => {
  ...
  const calcularSubtotal = (pedido: any) => {
    ...
    if (pedido.detallePedidoList) {  
      total += pedido.detallePedidoList.reduce((acc: number, detalle: any) => {   //acc = acumulador
        return acc + (detalle.articulo?.precio || 0) * detalle.cantidad;
      }, 0);
    }
    // Sumar promociones
    if (pedido.detallePromocionList) {
      total += pedido.detallePromocionList.reduce((acc: number, detalle: any) => {
          ...
          {cargando ? (
            ...
            <>
              ...
              <div className="bg-gray-50 p-6 border border-gray-300 mt-4 font-lato">
                ...
                <p className="font-lato mt-2"><strong>Email:</strong> {cliente.email}</p>
                ...
                <p className="font-lato -ml-0.5 mt-2">
                  <strong >Teléfonos:</strong> {telefonos.length > 0
                ...
                {direcciones.length > 0 ? (
                  <ul className="space-y-4 font-lato">
                    {direcciones.map((d, index) => {
                      ...
                      return (
                        <li key={d.id || index} className="border-b pb-3 last:border-b-0 font-lato">
                          {/* Alias de la dirección */}
                          <p className="font-bold text-lg font-lato">{d.alias || "Sin alias"}</p>
                  ...
                  <p className="text-gray-500 font-lato">No hay direcciones registradas.</p>
                )}
              </div>
              ...
              <div className="bg-gray-50 p-6 border border-gray-300 font-lato">
                ...
                {pedidos.length > 0 ? (
                    ...
                    {pedidos.map((pedido: any) => (
                      ...
                      >
                        ...
                        <div className="flex justify-between items-start mb-2">
                          <div>
                            <p className="font-bold text-lg font-lato">Pedido #{pedido.id}</p>
                          ...
                          <span className="px-3 py-1 rounded-full text-white text-sm bg-gray-400 font-lato">
                            {getEstadoTexto(pedido.estadoPedido?.nombreEstado || EstadosPedidosEnum.STANDBY)}
                          </span>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-2 text-sm mt-3 font-lato">
                          <p className="font-lato">
                            <strong>Total:</strong> {formatearPrecio(calcularTotal(pedido))}
                          </p>
                          <p className="font-lato">
                            <strong>Tipo:</strong> {pedido.tipoEnvio?.tipoDelivery || "N/A"}
                          </p>
                          <p className="font-lato">
                            <strong>Pago:</strong> {pedido.tipoPago?.tipoPago || "N/A"}
                          </p>
                          <p className="font-lato">
                            <strong>Sucursal:</strong> {pedido.sucursal?.nombre || "N/A"}
                          </p>
                        </div>
                        ...
                        {pedido.direccion?.direccion && (
                          <div className="mt-3 pt-3 border-t border-gray-200 font-lato">
                            ...
                            <p className="text-sm text-gray-600 font-lato">
                              {pedido.direccion.direccion.nombreCalle} {pedido.direccion.direccion.numeracion}
                            </p>
                          </div>
                        )}
                      </div>
                  ...
                  <p className="text-gray-500 font-lato">No hay órdenes registradas.</p>
                )}
              </div>
            </>
          )}
