...
const MisPedidos: React.FC<MisPedidosProps> = ({ pedidos }) => {
  ...
  const cargarPedidosDesdeBackend = async () => {
    try {
        const response = await axios.get(`${API_BASE_URL}/pedidos/all`);
        const pedidosMapeados: Pedido[] = response.data.map((pedidoDTO: any) => ({
            ...pedidoDTO,
            fecha: pedidoDTO.fecha ? new Date(pedidoDTO.fecha) : undefined,
            tiempoEstimado: pedidoDTO.tiempoEstimado ? new Date(pedidoDTO.tiempoEstimado) : undefined
        }));
