...
// Verificar si el artículo es un ArticuloManufacturado
function isArticuloManufacturado(
  articulo: ArticuloManufacturado | ArticuloInsumo
): articulo is ArticuloManufacturado {
  return "detalleInsumos" in articulo;
}
    ...
    <>
      ...
      >
        ...
        <div
          ...
          className="bg-white rounded-2xl grid grid-rows-[3fr_1fr] w-full max-w-lg"
        >
          {/**Imagen y tiempo */}
            ...
            {articulo.tiempoEstimado && (
              <div className="absolute bottom-0 bg-[#D93F21] m-auto text-center p-5 px-15 rounded-tr-2xl ">
                <h1 className="text-xl">
                  {`${articulo.tiempoEstimado.split(" ")[0]}-${
                    Number(articulo.tiempoEstimado.split(" ")[0]) + 5
                  }`}{" "}
                  min
                </h1>
              </div>
          ...
          <div className="w-11/12 m-auto text-black flex flex-col gap-2">
              ...
              articulo.detalleInsumos.length > 0 ? (
                <p>
                  ...
                  {articulo.detalleInsumos
                    .filter(
                      (detalle) =>
                        detalle.articuloInsumo && detalle.articuloInsumo.nombre
                    )
                    .map((detalle, index, insumos) => {
                      if (
                        !detalle.articuloInsumo ||
                        !detalle.articuloInsumo.nombre
                      )
                        return "";
            ...
            <div className="pb-2 flex justify-between">
              ...
              <button
                ...
                className="bg-[#D93F21] text-white px-2 rounded-lg"
              >
                Agregar a mi orden (${articulo.precio * cantidad})
              </button>
            </div>
          </div>
        </div>
